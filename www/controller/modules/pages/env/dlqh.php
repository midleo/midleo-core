<?php if(empty($thisarray['p2'])){ include "applist.php"; } else { ?>
  ...
  <div class="col-md-3 position-relative">
      <input type="text" ng-model="search" class="form-control  topsearch" placeholder="Find a DLQH rule">
...
<span data-bs-toggle="tooltip" data-bs-placement="top" title="Define new DLQH definitions"><button type="button" class="waves-effect waves-light btn btn-info" data-bs-toggle="modal" href="#modal-obj-form" ng-click="showCreateForm()"><svg class="midico midico-outline"><use href="/assets/images/icon/midleoicons.svg#i-add" xlink:href="/assets/images/icon/midleoicons.svg#i-add"/></svg>&nbsp;New</button></span>
  </div>
</div><br>
  <div class="card ">
...
<div class="row"><div class="col-md-12">
  ...
  <div class="modal" id="modal-obj-form" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
      ...
      <div class="modal-header"><h4>Define DLQH definition</h4></div>
...
<?php foreach($zobjfte as $val) { echo '<option value="'.$val["qmname"].'">'.$val['qmname'].' ('.$val['serverdns'].')</option>'; } ?>
 ...
 <?php } ?>  
            ...
            <div class="form-group row">
              ...
              <div class="col-md-9"><textarea ng-model="mq.name" class="form-control" ng-required="true" rows="3"></textarea></div>  
            </div>   
            
          </div>
          <div class="modal-footer" style="display:flow-root list-item;">
          <div class="float-start"><a href="https://www.google.com/search?q=ibm+mq+create+dead+letter+queue+rules" target="_blank" class="waves-effect waves-light btn btn-light btn-sm">Information about DLQH</a></div>
          <div class="float-end">
          ...
          <button type="button" id="btn-mqsc-obj" class="waves-effect waves-light btn btn-info btn-sm" ng-click="dlqh('<?php echo $page=="env"?$thisarray['p1']:$thisarray['p3'];?>',mq.proj,'<?php echo $_SESSION['user'];?>','<?php echo $page;?>')" ng-href="{{ url }}"><i class="mdi mdi-download"></i>&nbsp;Create rules</button>
