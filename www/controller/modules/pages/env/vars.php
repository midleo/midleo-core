<?php if(empty($thisarray['p2'])){ include "applist.php"; } else { ?>
  ...
  <div class="col-md-3 position-relative">
      <input type="text" ng-model="search" class="form-control topsearch" placeholder="Find a variable">
...
<?php if ($_SESSION['user_level'] >= 3) {?><span><button data-bs-toggle="tooltip" data-bs-placement="top" title="Export the objects in excel" type="button" class="waves-effect waves-light btn btn-light" ng-click="exportData('<?php echo $thisarray['p1']; ?>')">Export&nbsp;<svg class="midico midico-outline" ><use href="/assets/images/icon/midleoicons.svg#i-up" xlink:href="/assets/images/icon/midleoicons.svg#i-up"/></svg></button> </span><?php }?>
 <span data-bs-toggle="tooltip" data-bs-placement="top" title="Define new variable"><button type="button" class="waves-effect waves-light btn btn-info" data-bs-toggle="modal" href="#modal-obj-form" ng-click="showCreateForm()"><svg class="midico midico-outline" ><use href="/assets/images/icon/midleoicons.svg#i-add" xlink:href="/assets/images/icon/midleoicons.svg#i-add"/></svg>&nbsp;New</button></span>
  </div>
</div><br>
  <div class="card ">
...
<div class="row"><div class="col-md-12">
  <table class="table table-vmiddle table-hover stylish-table mb-0">
   ...
   <tbody ng-init="getAll('<?php echo $thisarray['p1'];?>','<?php echo $thisarray['p2'];?>','<?php echo $page;?>')">
      ...
      <tr id="contloaded" class="hide" dir-paginate="d in names | filter:search | orderBy:sortKey:reverse | itemsPerPage:10" pagination-id="prodx" ng-hide="d.varname==''">
      <td class="text-center">{{ d.proj }}</td>
      <td class="text-center">{{ d.varname }}</td>
        <td class="text-center">{{ d.varvalue }}</td>
        <td class="text-center">
        <div class="btn-group" role="group">
          <button type="button" ng-click="readOne('<?php echo $thisarray['p1'];?>',d.varid,d.varid,'<?php echo $page;?>')" style="" class="btn btn-light btn-sm bg waves-effect"><svg class="midico midico-outline"><use href="/assets/images/icon/midleoicons.svg#i-edit" xlink:href="/assets/images/icon/midleoicons.svg#i-edit"/></svg></button>
          <?php if($zobj['lockedby']==$_SESSION['user']){?>
          <?php if($_SESSION['user_level']>="3"){?><button type="button" ng-click="deletevar(d.varname,d.varid,'<?php echo $_SESSION['user'];?>','<?php echo $thisarray['p2'];?>')" class="btn btn-light btn-sm bg waves-effect"><svg class="midico midico-outline"><use href="/assets/images/icon/midleoicons.svg#i-x" xlink:href="/assets/images/icon/midleoicons.svg#i-x"/></svg></button><?php } ?>
          <?php } else {?>
          <?php if($_SESSION['user_level']>="3"){?><button type="button" class="btn btn-light btn-sm bg waves-effect"><svg class="midico midico-outline"><use href="/assets/images/icon/midleoicons.svg#i-x" xlink:href="/assets/images/icon/midleoicons.svg#i-x"/></svg></button><?php } ?>
          <?php } ?>
          </div>
        </td>
      </tr>
  ...
  <div class="modal" id="modal-obj-form" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
      ...
      <div class="modal-header"><h4>Variable</h4></div>
        <form name="form" ng-app>
          <div class="modal-body container" style="width:100%;min-height:120px;max-height:500px;overflow-x:hidden;overflow-y:scroll;">  
              ...
              <div class="form-group row">
                    ...
                    <div class="col-md-6"><input id="tags" data-role="tagsinput" type="text" class="form-control"></div>
                  ...
                  <div class="form-group row">
                    <label class="form-control-label text-lg-right col-md-5" ng-class="{'has-error':!mq.varname}">Variable</label>
                    <div class="col-md-7"> <input ng-model="mq.name" ng-required="true" type="text" class="form-control"></div>
                  </div>
            <div class="form-group row">
                    <label class="form-control-label text-lg-right col-md-5" ng-class="{'has-error':!mq.type}">Type</label>
                    <div class="col-md-7"><select class="form-control" ng-model="mq.vartype" ng-minlength="1" ng-required="true"><option value="">Please select</option><option value="envrelated">Different for each environment</option><option value="envsame" >Same for all environment</option></select></div>
                  </div>
                  <?php foreach($menudataenv as $key=>$val){  ?>
               <div class="form-group row" ng-show="mq.vartype=='envrelated'" >
                    <label class="form-control-label text-lg-right col-md-5" ><?php echo $val['name'];?></label>
                    <div class="col-md-7"><input ng-model="mq.env.<?php echo $val['nameshort'];?>"  type="text" class="form-control"></div>
                  </div>
               <?php } ?>
                 <div class="form-group row" ng-show="mq.vartype=='envsame'" >
                    <label class="form-control-label text-lg-right col-md-5" >Value</label>
                    <div class="col-md-7"><input ng-model="mq.varvaluesame" type="text" class="form-control"></div>
              </div>
