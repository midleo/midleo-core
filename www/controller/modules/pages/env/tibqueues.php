    ...
    <tbody ng-init="getAllTib('queue','<?php echo $thisarray['p2'];?>')">
      ...
      <tr id="contloaded" class="hide" dir-paginate="d in names | filter:search | orderBy:'srv':reverse | itemsPerPage:10" pagination-id="prodx" ng-hide="d.srv==''">
        ...
        <div class="text-start d-grid gap-2 d-md-block">
          <button type="button" ng-click="readOneTib('queue',d.id,d.proj)" style="" class="btn btn-light btn-sm bg waves-effect"><i class="mdi mdi-pencil mdi-18px"></i></button>
          <?php if($_SESSION['user_level']>="3"){?>
          <button type="button" ng-click="duplicate('tibco','queue',d.id,'<?php echo $_SESSION['user'];?>','<?php echo $thisarray['p2'];?>')" class="btn btn-light btn-sm bg waves-effect" title="Duplicate"><i class="mdi mdi-content-duplicate mdi-18px"></i></button>
          <button type="button" ng-click="deletetib('queue',d.id,d.name,d.proj,'<?php echo $_SESSION['user'];?>')" class="btn btn-light btn-sm bg waves-effect"><i class="mdi mdi-close"></i></button>
          <?php } ?>
          </div>
        </td>
      </tr>
...
<?php foreach($zobjfte as $val) { echo '<option value="'.$val["serverdns"].'">'.$val['serverdns'].'</option>'; } ?>
 ...
 <?php } ?>  
          ...
          <?php if($_SESSION['user_level']>="3"){?>
            <button type="button" id="btn-create-obj" class="waves-effect waves-light btn btn-info btn-sm" ng-click="form.$valid && createtib('queue','<?php echo $thisarray['p2'];?>','<?php echo $_SESSION['user'];?>')"><i class="mdi mdi-check"></i>&nbsp;Create</button>
            <button type="button"  id="btn-update-obj" class="waves-effect waves-light btn btn-info btn-sm" ng-click="updatetib('queue','<?php echo $thisarray['p2'];?>','<?php echo $_SESSION['user'];?>')"><i class="mdi mdi-content-save-outline"></i>&nbsp;Save Changes</button>
            <?php } ?>
           </div>
        </form>
      </div>
